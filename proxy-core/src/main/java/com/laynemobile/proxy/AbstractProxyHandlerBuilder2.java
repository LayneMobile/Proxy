...
public abstract class AbstractProxyHandlerBuilder2<T> implements Builder<T> {
    protected static final NamedMethodHandler handler(ProxyFunction<?> proxyFunction) {
    ...
    public abstract ProxyHandler<T> handler();

    public ProxyBuilder<T> builder() {
        return new ProxyBuilder<>(handler());
    }
