...
public class ProxyFuncTransformTemplate extends AbstractFunctionTemplate {
    private static final String FILE_NAME_TEMPLATE = "ProxyFunc${LENGTH}Transform";

    private final int func_length;

    public ProxyFuncTransformTemplate(String packageName, int length) {
        super(packageName, fileName(length), TEMPLATE, length);
    ...
    private static final String TEMPLATE = "" +
            ...
            "\n" +
            "public class ${FILE_NAME}<P, ${TYPE_ARGS}, R>\n" +
            ...
            "\n" +
            "    public ProxyFunc${LENGTH}Transform(Func${FUNC_LENGTH}<? super P, ${WILDCARD_TYPE_ARGS}, ? extends R> function) {\n" +
            "        super(function);\n" +
            "    }\n" +
            "\n" +
            "    public ProxyFunc${LENGTH}Transform(ProxyFunc${LENGTH}Transform<? super P, ${WILDCARD_TYPE_ARGS}, ? extends R> function) {\n" +
            "        super(function.function);\n" +
            "    }\n" +
            "\n" +
            "    public ProxyFunc${LENGTH}Transform(final Func0<? extends R> function) {\n" +
            ...
            "    }\n" +
            "\n" +
            "    public ProxyFunc${LENGTH}Transform(final R value) {\n" +
            "        super(new Func${FUNC_LENGTH}<P, ${TYPE_ARGS}, R>() {\n" +
            "            @Override public R call(P p, ${FUNCTION_PARAMETERS}) {\n" +
            "                return value;\n" +
            "            }\n" +
